        ...
        async function verifyUpdate(
            ...
            if (secondPrice) {
                expectedCumulativePrice = BigNumber.from(expectedCumulativePrice).add(
                    await calculateTimeWeightedPrice(mathUtil, BigNumber.from(secondPrice), BigNumber.from(deltaTime2))
                );
            } else {
                expectedCumulativePrice = BigNumber.from(expectedCumulativePrice).add(
                    await calculateTimeWeightedPrice(mathUtil, BigNumber.from(initialPrice), BigNumber.from(deltaTime2))
                );
            }
    ...
    describe(contractName + "#consultPrice(token, maxAge = 0)", function () {
        ...
        tests.forEach(function (price) {
            it(`price = ${price}`, async function () {
                ...
                expect(await accumulator["consultPrice(address,uint256)"](ethers.constants.AddressZero, 0)).to.equal(
                    price
                );
            });
        });
...
describePriceAccumulatorTests(
    "GeometricPriceAccumulator",
    "GeometricPriceAccumulatorStub",
    "GeometricPriceAccumulatorStubCaller",
    async (mathUtil, price, time) => {
        if (price.eq(0)) {
            // ln(0) is undefined
...
describePriceAccumulatorTests(
    "HarmonicPriceAccumulator",
    "HarmonicPriceAccumulatorStub",
    "HarmonicPriceAccumulatorStubCaller",
    async (mathUtil, price, time) => {
        if (price.eq(0)) {
