...
describe("FullMath", () => {
    ...
    describe("#mulDiv", () => {
        ...
        it("accurate with phantom overflow", async () => {
            ...
            expect(await fullMath.mulDiv(Q128, BigNumber.from(35).mul(Q128), BigNumber.from(8).mul(Q128))).to.eq(
                result
            );
        });
        ...
        it("accurate with phantom overflow and repeating decimal", async () => {
            ...
            expect(await fullMath.mulDiv(Q128, BigNumber.from(1000).mul(Q128), BigNumber.from(3000).mul(Q128))).to.eq(
                result
            );
        });
    });
    ...
    describe("#mulDivRoundingUp", () => {
        ...
        it("accurate with phantom overflow", async () => {
            const result = BigNumber.from(4375).mul(Q128).div(1000);
            expect(
                await fullMath.mulDivRoundingUp(Q128, BigNumber.from(35).mul(Q128), BigNumber.from(8).mul(Q128))
            ).to.eq(result);
        });
