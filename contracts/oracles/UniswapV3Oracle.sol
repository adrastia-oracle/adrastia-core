...
contract UniswapV3Oracle is IOracle {
    ...
    constructor(address uniswapFactory_, address quoteToken_, uint32 period_) {
        ...
        returns (uint256 price, uint256 tokenLiquidity, uint256 baseLiquidity)
    {
        ...
        WeightedOracleLibrary.PeriodObservation[] memory periodObservations = new WeightedOracleLibrary.PeriodObservation[](3);

        if (isContract(poolAddress500))
            periodObservations[0] = WeightedOracleLibrary.consult(poolAddress500, period);
        
        if (isContract(poolAddress3000))
            periodObservations[1] = WeightedOracleLibrary.consult(poolAddress3000, period);

        if (isContract(poolAddress10000))
            periodObservations[2] = WeightedOracleLibrary.consult(poolAddress10000, period);

        int24 timeWeightedAverageTick = WeightedOracleLibrary.getArithmeticMeanTickWeightedByLiquidity(periodObservations);

        price = OracleLibrary.getQuoteAtTick(timeWeightedAverageTick, uint128(10**(ERC20(token).decimals())), token, quoteToken);

        uint128 liquidity = periodObservations[0].harmonicMeanLiquidity + periodObservations[1].harmonicMeanLiquidity + periodObservations[2].harmonicMeanLiquidity;
