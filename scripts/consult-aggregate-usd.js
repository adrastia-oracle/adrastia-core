...
async function main() {
    ...
    for (const stablecoin of stablecoins) {
        ...
        underlyingOracles.push(uniswapV2);
        underlyingOracles.push(uniswapV3);
        underlyingOracles.push(sushiswap);
    }
    ...
    const oracle = await createAggregatedOracle(
        ...
        6,
        periodSeconds,
        underlyingOracles.flatMap((x) => x.oracle.address),
        tokenSpecificOracles
    );
    ...
    while (true) {
        try {
            for (const underlyingOracle of underlyingOracles) {
                if (await underlyingOracle.liquidityAccumulator.needsUpdate(token)) {
                    const updateTx = await underlyingOracle.liquidityAccumulator.update(token);
