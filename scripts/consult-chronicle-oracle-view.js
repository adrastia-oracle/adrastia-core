...
const wethFeedAddress = "0x46ef0071b1E2fF6B42d36e5A177EA43Ae5917f4E";

const authAddress = "0xc50dFeDb7E93eF7A3DacCAd7987D0960c4e2CD4b";
...
async function main() {
    ...
    if (authAddress) {
        const abi = ["function kiss(address who) external"];
        const auth = await ethers.getContractAt(abi, feed);

        // Impersonate authAddress
        await hre.network.provider.request({
            method: "hardhat_impersonateAccount",
            params: [authAddress],
        });
        const authSigner = await ethers.provider.getSigner(authAddress);

        await auth.connect(authSigner).kiss(oracle.oracle.address);
    }

    while (true) {
        const price = await oracle.oracle.callStatic["consultPrice(address)"](token);
